...
6. On a scale of 1 to 5, how satisfied are you with the overall mobile digital experience on campus, and what is the #1 improvement you would request?`);
  ...
  const handleGenerate = async (resumeFromIndex?: number) => {
    ...
    try {
      // Run the FIRST participant alone before fanning out. Two reasons:
      //  1. The evidence profile is built once per session and cached; if all participants start
      //     together they each rebuild it, which is several redundant LLM calls per run.
      //  2. If something is fundamentally wrong (bad guide, no provider) we fail after one
      //     participant instead of burning the whole batch.
      if (queue.length > 0 && !stopped) {
        const first = queue.shift()!;
        try {
          await runOne(first);
        } catch (e: any) {
          if (e instanceof PasscodeRequiredError) throw e;
          failures.push(`Participant ${first}: ${e?.message || e}`);
        }
      }
