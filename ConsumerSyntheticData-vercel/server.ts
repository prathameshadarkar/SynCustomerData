/**
 * server.ts — Express HTTP layer for the Synthetic Discussion-Guide Transcript Generator.
 *
...
import { storage, keys, readJson, writeJson, clientUploadsEnabled } from './server/storage';
import {
  ingestFile,
  removeFileFromIndex,
  clearIndex,
...
import { availableProviders, configuredProviders, logicalModel } from './server/llm';
import {
...
// Local / VM: mount the frontend (Vite in dev, static public/ otherwise) and listen.
// Vercel: the app is exported below and invoked per request; static assets are served by the CDN,
// and the static middleware is mounted synchronously so deep links still resolve.
if (IS_VERCEL) {
  void attachFrontend();
} else {
  attachFrontend().then(() => {
