 ...
 *   LLM_PROVIDERS=openai                          (tried in order, falls through on 429/5xx)
 *   LLM_PROVIDERS=cerebras,groq,cloudflare        (free-tier alternative)
...
export interface ChatRequest {
  ...
  taskName?: string;
  /**
   * Accept a response cut off by max_tokens instead of throwing. Only safe for plain-text tasks
   * where a partial answer is still usable (guide extraction, for example). Never set it alongside
   * `jsonSchema` — truncated JSON does not parse.
   */
...
const PROVIDERS: Record<string, ProviderDef> = {
  ...
  /** OpenAI proper. Paid tier: far higher rate limits than the free providers above.
   *  gpt-4o-mini is the cheap workhorse ($0.15/$0.60 per 1M tokens) and handles this workload well. */
  openai: {
    name: 'openai',
    baseUrl: () => process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1',
    apiKey: () => process.env.OPENAI_API_KEY,
    models: {
      'gpt-oss-120b': process.env.OPENAI_GENERATION_MODEL || 'gpt-4o-mini',
      'gpt-oss-20b': process.env.OPENAI_UTILITY_MODEL || 'gpt-4o-mini',
    },
    supportsJsonSchema: true,
    minSpacingMs: 0,
    tpmLimit: Number(process.env.OPENAI_TPM || 200000),
    maxOutputTokens: Number(process.env.OPENAI_MAX_OUTPUT_TOKENS || 8000),
    // NOTE: deliberately no extraBody — `reasoning_effort` is a gpt-oss parameter and
    // OpenAI rejects it as an unknown field on gpt-4o-mini.
  },
...
/**
 * Budgets are sized for the provider we expect to actually serve the request (the first configured
 * one), not the worst provider in the list. Otherwise adding a tiny free fallback silently shrinks
 * prompts and truncates transcripts even when the primary provider has plenty of headroom.
 * Set LLM_BUDGET_MODE=conservative to go back to min-across-all-providers behaviour.
 */
function budgetProviders(): string[] {
  const avail = availableProviders();
  if (avail.length === 0) return [];
  const mode = (process.env.LLM_BUDGET_MODE || 'primary').toLowerCase();
  return mode === 'conservative' ? avail : [avail[0]];
}

/** Per-minute token budget used to size prompts and evidence. */
export function tightestTpm(): number {
  const list = budgetProviders();
  if (list.length === 0) return 8000;
  return Math.min(...list.map((n) => PROVIDERS[n].tpmLimit));
}

/** Largest max_tokens to request. */
export function outputTokenCap(): number {
  const list = budgetProviders();
  if (list.length === 0) return 1300;
  return Math.min(...list.map((n) => PROVIDERS[n].maxOutputTokens ?? 4000));
}
...
async function callProviderOnce(p: ProviderDef, model: string, req: ChatRequest, forceJsonMode: boolean): Promise<{ text: string; usage?: any; truncated?: boolean }> {
  ...
  // A `length` finish_reason means we hit max_tokens and the JSON is almost certainly cut off.
  // Say so explicitly — otherwise this surfaces downstream as the much less useful
  // "no JSON object found".
  const finish = parsed?.choices?.[0]?.finish_reason;
  if (finish === 'length') {
    // Truncation is deterministic: the same prompt against the same provider will truncate at the
    // same place, so retrying it is pure waste. Plain-text callers can opt into keeping the partial
    // answer; JSON callers cannot, because a cut-off object will not parse.
    if (req.allowTruncated && !req.jsonSchema) {
      console.warn(
        `[LLM] ${p.name} hit max_tokens (${body.max_tokens}) on task=${req.taskName || '-'}; keeping partial output`
      );
      return { text: content, usage: parsed.usage, truncated: true };
    }
    console.warn(`[LLM] ${p.name} hit max_tokens (${body.max_tokens}) on task=${req.taskName || '-'}; output truncated`);
    throw new ProviderError(
      `${p.name}: output truncated at max_tokens=${body.max_tokens}. Raise depth limits or shorten the guide section.`,
      502,
      true,
      500
    );
...
export async function chat(req: ChatRequest): Promise<ChatResult> {
  ...
  if (providers.length === 0) {
    throw new Error(
      'No LLM provider is configured. Set at least one of OPENAI_API_KEY, CEREBRAS_API_KEY, GROQ_API_KEY, CF_API_TOKEN(+CF_ACCOUNT_ID), or LLM_PROVIDERS=mock for offline testing.'
    );
  }
